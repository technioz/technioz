...
type Platform = "web" | "ios" | "android" | "react-native" | "flutter";
type Scope = "mvp" | "standard" | "complex";
type Design = "simple" | "custom" | "premium";
type Backend = "none" | "basic" | "complex";
type Integrations = "0" | "1-3" | "4+";

const platformConfig: Record<Platform, { label: string; baseMin: number; baseMax: number }> = {
  web: { label: "Web App", baseMin: 8000, baseMax: 25000 },
  ios: { label: "iOS App", baseMin: 12000, baseMax: 30000 },
  android: { label: "Android App", baseMin: 12000, baseMax: 30000 },
  "react-native": { label: "React Native (iOS + Android)", baseMin: 15000, baseMax: 35000 },
  flutter: { label: "Flutter (iOS + Android)", baseMin: 15000, baseMax: 35000 },
};
...
export function CostCalculator() {
  ...
  const estimate = useMemo(() => {
    ...
    const min = Math.round((p.baseMin * s + d.min + b.min + i.min) / 1000) * 1000;
    const max = Math.round((p.baseMax * s + d.max + b.max + i.max) / 1000) * 1000;

    return { min, max };
  }, [platform, scope, design, backend, integrations]);

  const breakdown = useMemo(
    () => [
      { label: platformConfig[platform].label, min: Math.round(platformConfig[platform].baseMin * scopeMultiplier[scope]), max: Math.round(platformConfig[platform].baseMax * scopeMultiplier[scope]) },
    ...
    <div className="grid lg:grid-cols-[1fr_380px] gap-10 items-start">
      ...
      <div className="bg-white-200 rounded-sm border border-neutral-300 p-[32px] flex flex-col gap-8">
        ...
        <RadioGroup
          ...
          options={Object.entries(platformConfig).map(([value, { label }]) => ({ value, label }))}
        />
